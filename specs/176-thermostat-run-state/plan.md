...
1. **Shared** (`src/shared/binding-candidates.ts`): `THERMOSTAT_STATE_ALIAS` constant;
   `inferDataBindingCategory(thermostat, state)` returns `appliance_state` (the spec 152
...
2. **Aliasing** (`ui/src/components/equipments/bindingUtils.ts`):
   ...
   `THERMOSTAT_STATE_ALIAS`; `TYPE_CATEGORY_ALIASES.thermostat = { toggle_power: "power",
...
3. **Reader** (`ui/src/lib/thermostat-state.ts`): `thermostatPowerStateBinding`, preference
   state → legacy `power` when declared (or read) boolean → undefined.
...
5. **Optimistic** (`ThermostatCard.tsx`): per-alias clearing keyed on the mirror binding's
   ...
   not-yet-bound `state` alias when there is none; 90 s TTL as the backstop.
6. **Plugin** (`sowel-plugin-panasonic-cc`): `ON_DEMAND_DELAYS_MS = [10s, 45s]`, bump 2.3.2.
7. **Docs**: state paragraph in `docs/user/equipments.md` + `.fr.md` (Climate section).
...
- `bindingUtils.test.ts`: the four alias rules (boolean → state, wattage stays power,
  toggle_power order stays power, foreign outdoor key → outsideTemperature); full plan for a
  Panasonic-like device (spec 077 categories); `computeMissingBindings` offers `state`, not
...
- `ThermostatCard.test.tsx`: ON from `state` with a numeric `power`; OFF actually sent when
...
3. On the PAC: add the `state` binding via the equipment's missing-bindings panel (offered
   automatically).
