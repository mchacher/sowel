...
Three changes on one subject: what the arbiter does, and what it says, about a load it is not driving. A pool pump switched on at a wall button on the reference installation exposed all three in a single afternoon. It ran on grid import while the house exported more than 2.5 kW, the button that exists to take such a load back was inoperative, and the timeline drew a pump consuming 600 W as an empty cell.

- Fix (energy): **a suspension answers an event, not a standing condition** (issue #958, #962). A deferrable load running with no grant is read as somebody at a wall switch, and arbitration for it is suspended for two hours. But the detector armed on the situation rather than on the event that caused it, and the situation does not end when the suspension does: the load is still running, so a minute after each expiry an identical suspension began, for ever. Every claim in between came back denied, and a claim is exactly what would have resolved the contradiction the detector fired on. "Resume control now" was inoperative for the one case it exists for — on the reference installation, resumed at 11:32:26, suspended again at 11:33:28, every single time. The state a suspension answers is now remembered, and the same unchanged state cannot arm a second one; switch the load off and on again and that is a new event. A pending claim deliberately does **not** exempt a load, a first version of the fix having done so and left a load somebody had switched on unprotected for as long as a recipe kept asking for it — the protection is deferred by one delay, never removed. Two defects fell with it: a manual order cost four hours of suspension rather than two, the detector adding a phantom wall-switch event after the first expired, and a load past its suspension is now journaled as running outside arbitration, which is what keeps the timeline and the daily report from calling it idle while it draws.
- Fix (energy): **a denied claim is an audit event, not a state transition** (issue #959, #961). A refusal was read as "the load went idle", alongside a revoke, a release and a restart. For those three a claim that existed has ended; a refusal is journaled _instead of_ creating a claim, so there is nothing to end and nothing about the load has changed. The visible consequence was a pump suspended and running, correctly painted "running outside arbitration", wiped back to "at rest" by the next refused claim — an empty cell for a load consuming 600 W, for a whole afternoon. The same line corrects a quieter defect in the daily metrics, which closed a suspension span on any event able to change state, reasoning that the arbiter cannot deny a suspended load. It is the exact opposite: a refusal whose reason is the manual control is the one event proving the suspension still holds. Neither defect was reachable before pool-pump-schedule v1.8.3 made a recipe retry a refused claim every 15 minutes.
- Feat (energy): **"pilotage manuel" is a state of its own** (issue #960, #963). The arbiter has always had the state, and the load roster has always rendered it, but the timeline had no colour for it: a load under manual control borrowed the slate of "running outside arbitration" when it was running and the grey of "at rest" when it was not. A reader could not tell "the arbiter has stood down for two hours because somebody acted" from "running outside arbitration", which are two situations with two different remedies — only the first clears with a button. It is called manual control rather than suspended because what is suspended is the arbiter's control, not the load, and because the useful thing to say is who is driving. It reads the same whichever way the switch went, a first version having kept the grey for a load left off and made the roster pill and the timeline say different words about the same load at the same instant. Its hue is the house blue, the selection band on the timeline giving that token up in exchange, and the journal dot follows, a click on a cell scrolling to the row that explains it. No published figure moves: the daily metrics translate the new state back before counting it, deliberately, because those series are compared against their own history and re-baselining them to paint a cell differently would be the wrong trade.

---

## 1.70.x: What a caller is entitled to, and what yields first

### v1.70.0 — 2026-09-13 { #v1-70-0 }

Two fixes, and each draws a line that had never been drawn. A public health endpoint answered "is this alive" by handing over the engine's entire inventory to anyone who asked. A sensor tile too narrow for its three columns chose to sacrifice the readings — the one thing on it worth reading. The rest is maintenance, and most of it is supply chain: every GitHub-owned action pinned by commit SHA, an advisory cleared that no tool would have opened a pull request for, and one grouping rule so that a version-locked family is never again split into two halves that cannot install.

- Fix (api): **the health detail is served to an authenticated caller, liveness to everyone** (issue #926, #928). `/api/v1/health` sat in `PUBLIC_ROUTES` and returned the full engine inventory to any anonymous caller: the exact engine version, the id and status of every installed plugin, and the device counts. The version pins an instance to a precise release and the plugin ids reveal which protocols and vendor clouds are in play, so together the three fields are reconnaissance material. The route stays public, because `scripts/install.sh` polls it to decide when a fresh stack is ready and uptime monitors need an answer without credentials. What varies is the payload: anonymously it reports liveness alone, and the detail is served to a caller carrying a valid JWT or API token. The auth middleware skips `PUBLIC_ROUTES` entirely and never populates `request.auth`, so rather than restating its prefix logic inside the route, token verification is extracted into a shared helper with an `optionalAuth` layer on top — for public routes that vary with authentication instead of rejecting. A token that is absent, malformed or expired degrades to the anonymous payload and never yields a 401, so a stale credential cannot break a monitor. The interface rebuilds its alarm banner from `health.integrations` after every WebSocket reconnect, through what was a bare fetch carrying no token; it now goes through the api client, or the banner would have silently restored empty.
- Fix (ui): **the readings are the last thing a narrow sensor tile gives up** (#938). A sensor tile squeezed its readings into a column too small for them and then scrolled them sideways: "16.8°C" cut mid-value with a scrollbar under it, while a third of the tile sat empty to the left of the picto. The `1fr auto 1fr` grid the tiles share was not what broke it — `overflow-y-auto` on the readings was. An item that scrolls has an automatic minimum size of zero, so the track stopped asking for the width its content needs and collapsed to a bare `1fr` share: on a 224 px card, 39 px of empty spacer, 120 px of picto, and 39 px left for a column that needed 51. `min-w-fit` gives the column its minimum back and the picto may now scale down to 72 px, so the empty spacer yields first, the drawing slides left and shrinks next, and the readings are the last thing on the tile to lose any ground. Verified in a browser against a live instance: at a 224 px card the tracks go from 39/120/39 to 27/120/51 with nothing clipped, at 168 px the spacer is gone and the picto is down to 117 px, and a wide tile is unchanged.
- Maintenance (ci): **every GitHub-owned action pinned by commit SHA, as third-party actions already were** (#927, #929). Every third-party action in this repository was already pinned by SHA; everything still on a floating tag belonged to a GitHub-owned org, which made the split look deliberate without being written down anywhere. All 22 remaining references now carry a SHA and a trailing comment naming the tag it resolved from. A tag is a mutable pointer: `@v7` resolves to whatever commit the owner has it pointing at when the workflow runs, so a workflow pinned that way names code rather than pinning it. The blast radius is concrete — `release.yml` checks out through `actions/checkout` in jobs that also authenticate to ghcr.io, so code substituted under a moved tag would run holding registry push credentials, which is the position needed to publish a poisoned image to every installation. This is the threat model spec 089 already applies to plugin distribution, where a sha256 is mandatory for every registry entry and a mismatched tarball is refused, precisely because a version number is not a content guarantee. Dependabot updates SHA pins in place while keeping the version comment, so the pins do not go stale.
- Maintenance (deps): **the backlog cleared, and one grouping rule so it does not recur** (#943, #944, #945, #946, #949, #952, #954, #955). Dependabot had split vitest 5 into one pull request for `vitest` and one for `@vitest/coverage-v8`, and both were red: the coverage package peers on an **exact** vitest version, not a range, so each failed `npm ci` against the half of the upgrade the other one carried. Neither was landable alone, and neither run mentioned that the other existed. The split followed a rule that earns its keep — majors stay individual, so one blocked major cannot hold a group hostage — but that rule is wrong for a family peer-locked to an exact version, which is the lesson of the six un-installable eslint pull requests of #797 one layer up. A `*-vitest` group now collects `vitest` and `@vitest/*` with no update-type restriction at all: every update type, always one pull request. Landed with it, all of the weekly backlog — vitest 5 across the backend and the interface, the minor/patch and toolchain groups on both sides, eslint 10.10, typescript-eslint 8.70, fastify 5.12.3, lucide-react 1.44 and React 19.3.0.
- Maintenance (deps): **adm-zip 0.6.1, clearing an advisory nothing had opened a pull request for** (#953). Dependabot alert 142 (GHSA-vwc7-r8mq-g2x9, moderate, runtime scope) had been open for four days with nothing proposing a fix, because the advisory records no patched version: extraction following destination symlinks was reported against a range ending at whatever was newest at the time. 0.6.1 shipped two hours after that morning's Dependabot run and blocks extraction from writing through symlinks inside the target, but the advisory has not caught up, and Dependabot only opens a security pull request when it has a fix to point at. Sowel was never exposed: the advisory concerns AdmZip's own extraction helpers, which appear nowhere here, and backup restore reads each entry's buffer and writes it itself, after refusing symlink entries, confining the resolved path under the data directory and holding the extension to a whitelist — the spec 089 C2 hardening. Hygiene rather than an incident, which is why no code moved, and `npm audit` is back to zero.
- Maintenance (docs): **two unlisted decks on AI-assisted development** (#941, #942). Thirteen rules for building software with an AI, each shown with its real implementation in these repositories and its GitHub Copilot equivalent. Written for a talk and hosted here so it can be shared as a link: one cut aimed at numerical-computing engineers, one at developers, differing in the closing line of each rule, three worked examples and one extra trap about reviewing a diff out of reflex. Served verbatim as self-contained HTML rather than MkDocs pages, so each keeps its own layout and needs no nav entry, and kept out of the indexes three ways — absent from `nav`, `noindex` in the head, and the paths disallowed in `docs/robots.txt`. Unlisted, not private: the URL is enough to read them.
- Maintenance (plugins): pool-pump-schedule 1.8.3 in the registry (#951). A denied pump claim, after a manual order left an override active, was held for ever, so the pump never re-claimed solar surplus until the instance was restarted.

## 1.69.x: Declared once, rather than inferred from whatever arrived

### v1.69.0 — 2026-09-08 { #v1-69-0 }

Three features, and each replaces a definition the engine had never written down. A thermostat was whatever two vendors' devices happened to expose, and spec 176 was the bill for that. A press on a timed command meant one thing because it could only ever mean one thing. And a partition subtracted every submeter from the main meter on the assumption that all of them measure current that flowed through it, which is false the moment a circuit is fed by a second utility supply. The fourth change asks a related question of a number: a forecast score averaged over every hour of the year was measuring the night.

- Feat (equipments): **what a thermostat is, declared once** (spec 177, issue #919, #923). A Sowel thermostat was whatever the Panasonic and MCZ devices exposed: their keys — `nanoe`, `airSwingUD`, `profile`, `resetAlarm` — were the auto-binding list offered in the product UI, a device needed the raw key `targetTemperature` to be offered as a thermostat at all, and the card carried a branch per vendor. That is the cause behind spec 176, whose own words were "an alias is not a vocabulary". The core is now declared once, in one shared module: room temperature, setpoint, on/off state, power, operating mode, and an optional outside temperature. A device is a thermostat when it carries a `setpoint` or `set_setpoint` category rather than one manufacturer's spelling of it, the room temperature and the operating mode resolve from categories like the rest of the core (`operation_mode` and `set_operation_mode` join the taxonomy with a closed vocabulary of auto, heat, cool, dry, fan and off), and a thermostat binds **every** order its device exposes: the core canonicalised, everything else kept as extras under its own key. The card leads with the core and renders the extras generically from each order's type, so the `resetAlarm`, `stoveState`, `profile`, `fanSpeed` and `ecoMode` branches are gone and a vendor key nobody anticipated still gets a control. No data, plugin, binding or migration change: an existing thermostat keeps every binding and every control it had. Part 2, making the plugins publish the contract themselves, is #922.
- Feat (equipments): **pressing again asks for longer, then gives up** (spec 178, #911). Spec 174 gave the engine one gesture: a second press extends the window by the same configured length, for ever. That answers "not yet" and never "how much longer" — on the reference installation, getting an hour out of a gate configured at a quarter of one takes four identical presses, and the control cannot say what the next press will do, because every press does the same thing. The other half was missing too: cancel sends the revert, which closes the gate, and nothing meant "keep it open, stop counting". An equipment can now declare a **ladder** of window lengths, two to six of them, increasing, and a press walks up it. The first press acts and arms the shortest rung; each further press moves the deadline to now plus the next rung and dispatches nothing; and a press past the top **gives the deadline up without reverting**, so the gate stays open and nothing will close it. That last press is deliberately not the cancel button: "stop counting" and "close it now" are two intents and both keep their own control. The rung is persisted, so a restart cannot silently restart the climb and turn a give-up press into another hour, and a ladder edited under a running window re-places it by length rather than by index, a stored index meaning something else the moment the rungs move. The configured duration is forced to the first rung at write time, because two places claiming what the first press does is how they come to disagree. Everything without a ladder keeps spec 174 rule 3 exactly.
- Feat (energy): **a meter fed by a separate supply leaves the reconciliation** (spec 179, #909). Spec 173 declared one topology the reconciliation arithmetic could not see, a meter inside another one; this is the other. The by-usage partition and the live donut both compute "other" as the main total minus the sum of the submeters, which holds only while every submeter measures current that flowed through the main meter. A clamp on a circuit fed by a **second utility supply** — the case that raised it is an EV charging socket on another subscription — breaks that three ways at once, and enrolment being a blocklist, it lands in every reconciliation by default: the partition shows a slice the main total never carried, the residual is eaten down to that clamp, at 0 on the reference site, and spec 123 prices another supply's kilowatt-hours at the main tariff. Same design principle as spec 173, declare what is true of the switchboard rather than tick a "hide this" box: an equipment now says that it hangs off a different supply, and both surfaces render it **apart** — its own group, raw series, kilowatt-hours only, never in euros — while its card, its history and its charts stay untouched. The flag is refused on the main meter and on production types, since the reference cannot sit outside itself, and it is checked on the resulting pair of type and flag so a later retype cannot reach the forbidden state. A separate-supply meter cannot be a spec 173 parent either: nothing the partition renders can be "inside" a meter on another supply, so a containment declaration pointing at one is stored and simply left unused, and clearing the flag restores the subtraction untouched. Zone aggregation stays deliberately raw: the garage genuinely draws those watts, whichever meter bills them. Existing installations are unchanged.
- Feat (auth): **a standard user chooses which mode is on** (issue #912, #916). Spec 131 classified modes as configuration wholesale, and its edge-case table recorded a 403 for a standard user activating one. That conflated two things the spec separates elsewhere: what a mode **is**, and which mode is currently **on**. The same user was already trusted, one line above in the allowlist, with a zone command that turns every light in the house off, so refusing "put the house in Night" beside that was not a coherent boundary, and it left a partner or a teenager unable to do the one thing they do most. Activate, deactivate and apply-to-zone are now reachable by a standard user. The honest argument for it is delegation, not harmlessness: activating a mode does more than flip a flag, since a mode's impacts can enable, disable or re-parameterise a recipe instance, and deactivating does not undo that. But an administrator authors those impacts, so a standard user chooses **when** a mode runs and never **what** it does — the same delegation the calendar and a physical button bound to a mode have always carried, neither of which asks for a role at all. Gating the recipe actions on the activator's role was the alternative and is worse: the same mode would half-apply depending on who pressed it. What a mode is stays admin-only, create, rename, delete and the impact routes untouched, and the add, edit and delete controls stay behind the admin check in the interface. Apply-to-zone gains the audit row it lacked, being the one of the three that dispatches orders without leaving a trace.
- Fix (energy): **the solar forecast is scored on the day's energy, not on an average of hourly powers** (issue #907, #908). The accuracy figure read "± 73 W over 168 hours" beside a 3.4 kW peak and a 23 kWh day, which is what prompted the report. The unit was right, both sides of the comparison being hourly mean powers, but the metric was not: averaged over every paired hour it is dominated by the night, where both sides are zero and the error is trivially perfect. On the reference site that is 74 of 168 hours, halving the figure, and the dilution is seasonal, so the score improves every autumn on its own and cannot be compared with itself. The headline is now the error on the daily energy integral, in kilowatt-hours and as a share of what was actually produced, over complete local days: 1.15 kWh and 5.2% on the same installation, against a cumulative bias of −0.7%. An integral cannot be diluted by hours that produce nothing. The running day is not dropped for being incomplete but reported on its own line, both sides summed over the hours elapsed, which is the only fair way to compare it and answers "is today on track" — and it names its own source, the forecast issued yesterday, because the two "Today" figures come from different forecasts and wore the same word. The percentage is omitted rather than printed as 0% when nothing was produced to take a share of, a dead inverter under a live forecast having looked flawless at the one moment the figure has to shout. The window selector gains one and three days and opens on three, a week of hourly points being a picket fence; the accuracy window is floored server-side at seven days so zooming in cannot blank the score; and the forward curve is capped to the selected window, so a one-day zoom is no longer one day of history against five of forecast.
- Fix (ui): **a hand-picked icon follows its equipment, and the timed tile stops clipping its own button** (#910). A custom widget icon was drawn everywhere from the frozen preview props that exist for the picker's thumbnail, so every surface showed it in one fixed state: the air compressor and the 3D printer added in v1.66.0 stayed at rest whatever the plug was doing, and a socket given the "Prise" icon rendered lit for ever. A hand-picked drawing now inherits the state props of the type icon it replaces, applied once in the resolver from the same value that drives the state pill, so the icon and the pill cannot disagree and there is no second on/off rule to keep in sync. Booleans are carried across families, since the picker offers every icon under "Other icons", while numeric state is left alone, a level and a position not sharing a value space. The 3D printer keeps its own four-valued prop, a plug only ever knowing whether the machine is powered. Separately, the spec 174 timed tile stacked its illustration, state pill and control row 14 px past the bottom of the shared 160 px shell, whose `overflow-hidden` then cut the arm button in half — the one thing on the tile you are meant to press. The rows now have a priority: the illustration is the only thing dropped at phone width, and the state and the controls always land inside the card. On a 240 px desktop tile nothing moves at all.
- Maintenance (core): **a missing `.js` import extension is refused by the linter, not by the container** (#924). The backend is an ES module and runs `node dist/index.js`, so Node demands an explicit extension on every relative specifier — but `moduleResolution: "bundler"` lets `tsc` accept one without, and vitest resolves it too. A missing `.js` therefore compiles, lints, passes all 2556 backend tests, and throws `ERR_MODULE_NOT_FOUND` when the container boots. The thermostat review above caught exactly that, by hand, on a shared module the interface also imports; nothing in CI would have, and nothing would have caught the next one. An eslint rule now refuses a relative import, re-export or `export *` without an extension, in the backend only, type-only imports included, since dropping the `type` keyword from an exempt import is precisely the edit nobody reviews.
- Maintenance (ci): **three developer-machine hooks that failed for reasons unrelated to the change being pushed** (#917, #918, #920). The specs-index check spawned two processes per spec folder per index, 700 of them at 175 folders, taking 13 s on macOS and blowing vitest's default timeout — so `npm run validate` and the pre-push hook were red on a developer machine while CI stayed green on Linux, and a hook that fails for an unrelated reason is a hook people learn to bypass. Set membership does not need a process per element: 13 s became 0.06 s, and two sibling scripts with the same shape followed, `check-specs-index.sh` in full going from 1.81 s to 0.07 s and `check-specs-complete.sh` from 1.87 s to 0.14 s. Behaviour is unchanged and the fourteen tests covering both assertions still pass. The secret scan, meanwhile, tested for the Docker CLI before falling back to it, which is true on any machine where Docker Desktop is installed and not running: the fallback then failed to connect and blocked every commit until Docker was started. It now asks the daemon, the cheapest question that actually means "can I run a container right now".
- Maintenance (docs): **documentation screenshots are shot on a throwaway local instance** (#914, #915). The screenshot pipeline depended on a Raspberry Pi demo host reachable over SSH; a box that has to be kept alive, reachable and up to date for something needed a few times a year is a liability, every session began by repairing it, and it has now gone away entirely. It is replaced by a disposable Sowel on the developer's own machine, on its own port, project name and volumes so tearing it down cannot touch anything else, and with no Docker socket mounted, since that mount grants effective root and self-update is not needed here. The agent guide also now names the three repositories of the public demonstrator and the project map that holds its decisions, together with the rule that keeps the product clean: the demo never justifies a core change on its own.
- Maintenance (plugins): panasonic_cc 2.3.2 in the registry (#906), the companion release to spec 176.
...
- Fix (equipments): **an order is checked where it was actually sent** (#901, #902). Confirming an order means watching for the ordered value to come back, and the reading to watch was picked by alias alone, which assumes an alias means one thing per equipment. On a submetered appliance it means two: `power` is the boolean sent to a cloud thermostat and, at the same time, the wattage read from the clamp on its supply. The engine compared `true` to `646` on every order, which no poll could ever make true, so the watchdog expired every time and raised an alarm. Measured on one production instance over fifteen days: fifteen power orders, fifteen alarms, all of them false, on a heat pump that obeyed every one of them. The mirror is now resolved in order of preference, a binding on the very device the order was sent to, then the binding on another device when its type could report the value, then the ordered device's own state under the order key, which is where a cloud thermostat publishes what nobody bound. That last step is what makes such an order confirm rather than merely stop alarming. When none of the three exists the order is still tracked and replayed, but it stays out of the alarm surface, because an alarm nothing can resolve is noise. A device genuinely reporting the opposite state still alarms, which is the whole point of the feature.

### v1.67.0 — 2026-09-04 { #v1-67-0 }

A wattage is the reading that lies quietly, and v1.65.0 started teaching the interface to distrust it. This release finishes the job by asking the only question that was still missing: how often does this source actually speak? Every freshness rule in the engine used a constant, and a constant has to be wrong for somebody: a Shelly at 1 Hz and a cloud poller on a five-minute cycle cannot share one window. The rest of the release is about failures that happened in silence, an update that pinned an instance for six hours without saying a word, and two barriers moved to where they can still be answered cheaply.

- Feat (equipments): **a power reading is judged on the cadence of its own source** (spec 175, #883, #889). Four surfaces decided whether a wattage could be drawn as a live measurement, and they disagreed with each other: a meter three minutes into a healthy 300 s cycle was silent in the Live banner, "outdated" on its Dashboard tile, and dropped from its zone total, at one instant. All four already called the same classifier; what differed was the budget handed to it, and that budget came from the equipment's **type**. A type says nothing about how often a device speaks, `main_energy_meter` covering both a 1 Hz meter and a 300 s poller. The window now comes from what the source does: the median of the last ten intervals between arrivals, falling back to the integration's declared polling interval, falling back to a conservative ten minutes while neither is known yet. The budget is 2.5 times that cadence, floored at two minutes and capped at thirty, resolved once and carried on the binding, so every surface compares an age to the same number by construction rather than by discipline. Detection follows the source: a 1 Hz meter that dies surfaces in two minutes where it used to take ten, and a five-minute poller keeps its quiet. The factor is 2.5 and not 2 because a "300 s" poll routinely lands at 305 to 320 s, and at exactly twice the cadence a healthy source sits on the boundary and oscillates.
- Fix (ui): **the Live energy banner reads the measurement, not a clock alone** (#881, #882). It kept announcing that the grid or production reading had been frozen for three minutes, on meters that were working, and the screen could not settle the argument: power is rounded to the nearest 5 W below 1 kW, so a figure displayed as 2.4 kW can swing by 49 W without a pixel changing. The banner asked one question, and it was never about the measurement: how long since the last message arrived, against a hard-coded 120 s calibrated for a meter that streams every second. Both polled sources in the registry report on a 300 s cycle, so the banner lit three minutes out of every five, permanently, and got learned as noise. Silence is now allowed ten minutes, twice the slowest supported cadence, and a stuck value gets its own verdict, read from the timestamp of the last change of the stored value at full precision. A source that republishes a cached figure forever satisfies every arrival check ever written, and until now nothing asked.
- Fix (self-update): **an update that fails says so, whether the helper dies or never answers** (#880, #884, #885). Found on a live instance stuck on "update in progress" for six hours. The engine spawns a helper container and assumes the helper always stops it, so nothing cleared the `updating` flag when the helper died first: the full-screen overlay stayed up for the life of the process and every later update or restart answered 409, with a manual `docker restart` the only way out. On 2026-09-02 a ghcr.io timeout did exactly that. The engine now watches the helper it spawned, and reaching that callback means the swap did not happen, since a successful helper recreates Sowel and this process dies first. It releases the flag and reports the failure with the helper's last lines, in the overlay's own slot rather than by dropping the spinner, which was indistinguishable from an update that worked and forgot to reload. The second half covers the helper that never answers at all, a pull against a registry that accepts the connection and goes quiet: the wait is now raced against a fifteen-minute watchdog, far past a normal pull on a slow link. Giving up does not kill the helper, which may still be halfway through recreating the stack; it is removed only once it has actually exited.
- Fix (ui): **a refused plugin action lands under the row** (#880). Enable, disable and uninstall caught their errors and ignored them outright, and the update path dropped everything it did not recognise. The refusal now appears under the row and in the detail sheet, which is where uninstall lives. The case that exposed it: renaming a personal package's GitHub repository. GitHub redirects the old name, so the old package id keeps advertising an update that can never install, and the server's explanation never reached the screen.
- Maintenance (plugins): **six MQTT integrations shipped the same connection fix** (#891, and one report worth reading, `sowel-plugin-zigbee2mqtt#19`). A broker not yet reachable when Sowel boots is an ordinary startup race, but it left the plugin frozen on "disconnected" for its whole lifetime: device data stopped at the first retained message and orders were never dispatched, until the container was restarted. The subscriptions issued on the closed socket were never retried either, and since the client uses clean sessions that loss also happened after any later reconnection, not only at boot. The same file lived in six repositories, five of them byte for byte, so all six are released: zigbee2mqtt 2.7.1, somfy-rts 1.1.1, tasmota 1.0.7, shelly_mqtt 1.2.3, displays 0.2.2, apsystems 0.1.1. Update them from the plugins page.
- Maintenance (ci): **two gates moved to where they can still be answered cheaply** (#872, #886, #892, #893). A missing specs-index row used to fail at the tag, charging a release for a gap the author could have closed with one table row: v1.64.0 and v1.65.0 were both blocked that way, and both tags had to be force-updated. That half of the check now runs on the pull request, in both indexes, and prints the row to paste in the right language. The other gate is new: nothing verified that a registry entry's SHA256 was the hash of the tarball GitHub actually publishes for the version beside it, and a wrong pair breaks the install on every instance about an hour later, once the CDN has propagated. A bumped entry is now checked against its published asset, hash and manifest, on the pull request that bumps it. The release procedure itself was two checks behind what CI enforces, and now runs the same set (#894).

---

## 1.66.x: What the engine could not say

### v1.66.0 — 2026-09-01 { #v1-66-0 }

Three features, and each of them started as something the engine had no words for. A breakdown whose arithmetic assumed that two clamps never measure the same current, when a switchboard is exactly where they do. A gate that had to be opened for fifteen minutes, which no equipment could express, so three recipes each kept their own clock and already disagreed about when to cancel it. And a Dashboard card that printed what a click would do, and did nothing when clicked.

- Feat (energy): **a meter can declare that it sits inside another one** (spec 173, #873). The by-usage breakdown splits the household total into slices and calls the rest "other", which holds only while the submeters are disjoint. The case that raised it: a guest house measured by one clamp, and its water heater, fed from that same board, by a second one. Both enrol as submeters, so the heater's kilowatt-hours landed in two slices and the residual lost the same amount, 2.09 kWh double-counted out of a 12.3 kWh day on one night cycle. Nothing could be configured around it: the only levers were deleting the measurement or switching off its historisation, a wrong number traded for a lost history. An equipment now carries `meteringParentId`, "my consumption is already counted by that meter", and the breakdown renders each parent minus its _direct_ children, which is what makes a chain add back up: for A ⊃ B ⊃ C, A−B plus B−C plus C is exactly A. The live donut on the same page follows the same rule, a child with no live reading subtracting nothing rather than spending "we do not know" as a number. What does not change is every place a meter is read as a sensor: the equipment card, the cumulative counters, the history API and the zone aggregation keep the raw measurement, because a card contradicting its own sensor would be worse than the bug. Only the partition subtracts, and it says so, "net of its submeters". The picker never offers an option that would fail, the route refuses a cycle by walking the graph the declaration would create rather than the pair, and deleting a parent clears the declarations inside it.
- Feat (equipments): **the engine holds "act now, revert after N"** (spec 174, #875). Nothing could express it, so every instance of it was a recipe holding its own clock: motion-light, state-trigger-light, delivery-gate, three copies of one deadline with three sets of cancellation rules. An equipment now carries at most one revert the engine owes it, and when. The action stays an ordinary order, inheriting inversion, value resolution and delivery confirmation untouched; what is added is the one thing that was missing, something that remembers across a restart that the gate in the yard is open. The deadline is a row and not a timer in memory: one still ahead is re-scheduled on its remainder at boot, and one that passed while the engine was down fires on the way up, that outage being the case the feature exists for. Four rules were decided rather than discovered in production. A revert done by hand disarms the window, because firing later would undo the user's own hand. A second arm of the same action moves the deadline and dispatches nothing: "open again", from somebody looking at an open gate, means "give me more time". A revert that could not be sent raises an alarm and stops, the engine having no way to know whether a second send would put the equipment back or act on it again. And deleting the equipment takes the deadline with it. The command is configured per equipment by an administrator and armed from the equipment page, from a second tile pinned beside the ordinary one, or from the compact row, all three rendering one countdown that reads the engine's deadline on every tick rather than counting down from a number captured when the page opened. An equipment is only eligible if it carries both the order and a state reading tied to it: without one, a revert done by hand could never disarm the window, and the deadline would act on an equipment that has moved since.
- Feat (ui): **a click anywhere on a recipe tile fires its control** (spec 171, #868). Spec 169 gave a recipe instance a Dashboard tile and left the whole card inert: the only thing that acted was the pill at its foot, on a card that said "Ready — one click opens the Gate for 15 min". Every other widget on that Dashboard has actuated on a card click since spec 098. A tile rendering exactly one control now fires it from the whole card, same cycle and same next value, with the pill still there for anyone who prefers to aim; two controls and the card stays inert, since it would have to guess which one, and so do zero controls, a disabled instance and edit mode. Because that turns a 240 px square into a gate opener, the recipe declares what should be asked first. Reaching for `tile.confirmFrom`, naming the equipment slot the control actuates, is the answer that scales: that equipment's own "Confirmation before action" then decides alone, so the question is answered once, on the gate, for every surface that actuates it, and a user who turned the guard on for their Portail no longer gets a tile that fires on a tap. Where no equipment can be derived, a boolean slot named in `confirmParam` lets the instance decide, and `tile.confirm` remains the package's default for an instance never asked. On mobile the guarded card opens the spec 146 slide-to-confirm sheet naming the position it is about to switch to; the pill is never guarded and desktop never confirms, a small target and a mouse click being deliberate already.
- Feat (ui): **two workshop icons, and a printer that shows which of its four states it is in** (#876). A smart plug in a workshop most often drives a compressor or a 3D printer, and the picker had a shape for neither, so both wore a wall socket. The printer is off, powered and idle, printing, or in fault, and the last one is what a plug on it is watched for, so it takes a state rather than a boolean: the fault reads as a badge and not as a colour alone, so it survives a colour-blind eye and a 48 px picker cell.
- Maintenance: the agent guide asks for short, ordered answers in the terminal (#877).

---

## 1.65.x: A total, and the readings it refuses

### v1.65.0 — 2026-08-31 { #v1-65-0 }

One number arrives on the zone header, and most of the work here went into deciding what may enter it. A wattage is the reading that lies quietly: a clamp that stopped reporting keeps its last value, and a stale `0 W` reads as an appliance that is off rather than as a question. Three of the four changes are about that, and the fourth is about a panel that could ask a question but not accept the answer.

- Feat (zones): **a zone totals the power its meters measure** (spec 170, #866, #867). A zone could say how warm it is and whether anyone is there, but not how much this part of the house is drawing, although every watt of the answer was already in the engine. The case that surfaced it: a guest house metered by two clamps, one on the flat's own feed and one on a hob fed from the main board, so two equipments in one zone that nothing could add together. No single equipment can carry both readings, a binding alias being unique per equipment, and charts have no arithmetic between series. The zone header now carries a power pill summing the zone and all its descendants, through the same accumulator merge the water flow total already used. What may enter that sum is decided by rules the engine already applies elsewhere rather than by new ones: the grid total and the production meters never count, since that is exactly what separates a submeter from a house balance in the energy breakdown, and a reading past its freshness budget is dropped rather than counted as zero. A zone with no meter shows no pill, which is not the same as a zone whose meters all read `0 W`, and a clamp mounted backwards is summed as it reports, negative, because a negative total is a wiring fault worth seeing. The first implementation carried three defects, fixed in the same release (#867): a Legrand NLPC meter has no `power` channel at all, its live reading being `demand_5min`, so summing `power` alone read nothing rather than something stale and the meter dropped out of the total while its own card kept printing live watts; the freshness rule had no clock, the aggregator recomputing only when an equipment reports and a clamp that went quiet reporting nothing, so in the exact case the feature was written for, a zone holding two meters and nothing else, the total stayed frozen at its last value indefinitely, and a 60 s tick now re-judges the zones that carry readings; and the sum is rounded to whole watts, tenths never having been displayed while guaranteeing that sub-watt jitter on an idle plug published an event up the whole chain of parent zones on every meter report in the house.
- Fix (ui): **an outdated wattage is no longer drawn as a live measurement** (#865, issue #839). Eight render sites across five files printed watts without ever consulting the age of the reading. The engine's own `stale` flag cannot answer that question, and deliberately so: the electrical window applies only to declared meters, because a tight window on a steady load would flag a healthy appliance as degraded on every reporting cycle, so a water heater carrying a power channel reports `stale: false` however old its value is. Measured on production: 560 W drawn and `0 W` displayed, the reading 944 s old, and a wood stove carrying a value 124 days old. The rule already existed and already served the live breakdown and the submeter feed; what was missing was a way for the tiles to ask it. They now do, and a reading that cannot be presented as current is replaced by a dash with its age beside it rather than by a number. Two sources get a looser budget because they cannot be fresher by construction: a `demand_5min` power is already averaged over five minutes, and the one solar integration in the registry delivers on a 300 s Tasmota cadence.
- Fix (ui): **a personal package updates from the top bar panel** (spec 172, #869). A package installed from your own GitHub source is only updated after you re-pin the fingerprint of the new version, which is the guarantee spec 136 exists to give. The updates panel offered an Update button for those packages, then printed the server's refusal as a raw error code in red, because it did not know how to raise the fingerprint dialog. The only way through was Administration then Plugins, so the panel built to be the one place updates happen was the one place a personal update could not. The dialog moves out of the Plugins page into a shared component and both surfaces raise the same one. The gate itself does not move: a new release is new content, and its fingerprint is still confirmed per version. What changes is only where the question can be answered.
- Maintenance (docs): fourteen screenshots refreshed across the documentation, an English page that was showing French, and one leak the anonymised fixtures had missed (#863, #864).
