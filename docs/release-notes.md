...
- Maintenance (api): **the last hand-rolled request bodies are schema-validated** (#837, issue #597). Widget bodies were deferred because they are conditional on the widget type; existence lookups stay in the handler and keep their original status, since converting a shape check is not a reason to renumber an answer a client may depend on.
