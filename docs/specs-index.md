...
## V1.5 — energy submeters, MQTT toggles, state-trigger recipe
...
| 091 | Power-only submeters + by-usage chart | ✅     | `energy_meter` equipments accept devices that expose only `power` (W). New `SubmeterIntegrator` performs trapezoidal integration to a Wh stream persisted across restarts. New `GET /energy/by-usage` endpoint and a Total / By usage toggle on the Energy page rendering a stacked breakdown per submeter + an "Other" residual. |
