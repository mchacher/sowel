...
On a thermostat, the on/off state reported by the device itself is bound under the `powerState` alias. This matters on a **submetered** thermostat (a heat pump measured by a clamp): there the `power` alias carries the live wattage, and the wattage cannot tell the UI whether the unit is on or in standby. If your thermostat card never shows as ON, open the equipment's missing-bindings panel and add the device's power reading: it is offered as `powerState`. Legacy thermostats with the boolean bound directly under `power` keep working unchanged.
