...
- **A human at the wall switch** → _switch confirmation_ detects sustained divergence between decision and reality and suspends arbitration for that load; the journal says so. A suspension answers that **event**, not the standing state it leaves behind: a load left running outside arbitration earns one suspension, and the same unchanged state never arms another. Otherwise the manual-override delay expired into an identical suspension a minute later, for ever, and the load could never be taken back — "resume control now" included, which was undone 62 s after every press. A claim waiting on that load changes nothing: a recipe claiming through the whole solar day would otherwise leave a load somebody switched on unprotected for the day, and the arbiter would revoke it out from under them on the first deficit. The protection is deferred by one delay, never removed. Once it lapses the load is journaled as running outside arbitration, so the timeline and the day report say what it is actually doing instead of calling it idle.
