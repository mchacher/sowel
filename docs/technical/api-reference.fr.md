...
| -------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST`   | `/api/v1/equipments/:id/timed-action` | Agit maintenant, revient en arrière à l'échéance. Body : `{ alias, value, revertValue, durationMs }`, ou vide pour armer ce que l'équipement a configuré. Retourne l'action armée, qui porte `stepIndex` et `nextDurationMs` (spec 178). **Spec 178** : avec des paliers `durationStepsMs` configurés, un appui supplémentaire monte au palier suivant, et un appui au-delà du dernier répond `{ "disarmed": true }` — l'échéance est abandonnée et RIEN n'est envoyé. |
...
Un client doit le consulter avant de dessiner un segment. Une mesure hors budget est un reliquat, pas une mesure, et l'erreur est silencieuse : un `0 W` périmé ressemble exactement à un appareil éteint. Le budget est le `freshnessBudgetMs` de la liaison elle-même, dérivé de la cadence à laquelle cette source remonte (spec 175) : un compteur est ainsi jugé sur son propre comportement, et non sur une constante choisie pour son type d'équipement (issues #744, #832 et #883).

Les liaisons de catégorie `power` portent ce `freshnessBudgetMs: number` sur toutes les routes équipements, pas seulement sur ce rôle : c'est l'âge maximal auquel cette mesure peut encore être affichée comme une mesure en direct. Il vaut `clamp(2,5 x cadence, 120 s, 30 min)`, la cadence étant l'intervalle médian entre les arrivées récentes, ou à défaut l'intervalle d'interrogation déclaré par l'intégration. Un compteur qui remonte à 1 Hz porte donc 120 000, un connecteur cloud sur un cycle de 300 s porte 750 000. Le champ est absent quand le moteur n'a pas pu le résoudre (source muette depuis le dernier redémarrage, sur une intégration qui ne déclare aucun intervalle) : un client doit alors retomber sur 10 minutes, jamais considérer la mesure comme sans limite.
...
| `GET`  | `/api/v1/plugins/pages`          | Spec 180 — les pages proposées par les plugins installés : `{ pluginId, label, icon, placement, equipmentTypes, entryUrl }`. |
