...
## 1.69.x : Déclaré une fois, plutôt que déduit de ce qui arrivait

### v1.69.0 — 2026-09-08 { #v1-69-0 }

Trois fonctionnalités, et chacune remplace une définition que le moteur n'avait jamais écrite. Un thermostat était ce que les devices de deux fabricants exposaient, et la spec 176 en a été la facture. Un appui sur une commande temporisée ne voulait dire qu'une chose parce qu'il ne pouvait jamais en dire une autre. Et une répartition soustrayait tous les sous-compteurs du compteur principal en supposant qu'ils mesurent tous du courant passé par lui, ce qui est faux dès qu'un circuit est alimenté par un second réseau. Le quatrième changement pose la même question à un chiffre : un score de prévision moyenné sur toutes les heures de l'année mesurait la nuit.

- Fonctionnalité (equipments) : **ce qu'est un thermostat, déclaré une fois** (spec 177, issue #919, #923). Un thermostat Sowel était ce que les devices Panasonic et MCZ exposaient : leurs clés — `nanoe`, `airSwingUD`, `profile`, `resetAlarm` — formaient la liste d'auto-liaison proposée dans l'UI produit, un device devait porter la clé brute `targetTemperature` pour être seulement proposé comme thermostat, et la carte avait une branche par fabricant. C'est la cause derrière la spec 176, dont les propres mots étaient « un alias n'est pas un vocabulaire ». Le cœur est désormais déclaré une fois, dans un module partagé : température de la pièce, consigne, état marche/arrêt, puissance, mode de fonctionnement, et une température extérieure optionnelle. Un device est un thermostat dès qu'il porte une catégorie `setpoint` ou `set_setpoint` plutôt que l'orthographe d'un fabricant, la température de la pièce et le mode se résolvent par catégorie comme le reste du cœur (`operation_mode` et `set_operation_mode` rejoignent la taxonomie avec un vocabulaire fermé : auto, chauffage, froid, déshumidification, ventilation, arrêt), et un thermostat lie **tous** les ordres de son device : le cœur canonisé, tout le reste conservé en extras sous sa propre clé. La carte mène avec le cœur et rend les extras génériquement d'après le type de chaque ordre : les branches `resetAlarm`, `stoveState`, `profile`, `fanSpeed` et `ecoMode` ont disparu, et une clé fabricant que personne n'avait anticipée reçoit quand même un contrôle. Aucun changement de donnée, de plugin, de liaison ni de migration : un thermostat existant garde toutes ses liaisons et tous ses contrôles. La partie 2, faire publier le contrat par les plugins eux-mêmes, est l'issue #922.
- Fonctionnalité (equipments) : **appuyer encore demande plus longtemps, puis abandonne** (spec 178, #911). La spec 174 n'avait donné qu'un geste au moteur : un second appui prolonge le créneau de la même durée configurée, indéfiniment. Cela répond « pas encore » et jamais « combien de temps encore » — sur l'installation de référence, obtenir une heure d'un portail réglé à un quart d'heure demande quatre appuis identiques, et le contrôle ne peut pas dire ce que fera le suivant, puisque tous font la même chose. L'autre moitié manquait aussi : annuler envoie le retour, donc referme le portail, et rien ne voulait dire « laisse-le ouvert, arrête de compter ». Un équipement peut désormais déclarer des **paliers** de durées, de deux à six, croissants, et un appui les gravit. Le premier appui agit et arme le plus court ; chaque appui suivant repousse l'échéance à maintenant plus le palier suivant, sans rien envoyer ; et un appui au-delà du dernier **abandonne l'échéance sans revenir en arrière**, si bien que le portail reste ouvert et que plus rien ne le refermera. Ce dernier appui n'est volontairement pas le bouton d'annulation : « arrêter de compter » et « fermer maintenant » sont deux intentions et chacune garde son contrôle. Le palier est persisté, pour qu'un redémarrage ne relance pas la montée en silence et ne transforme pas un appui d'abandon en une heure de plus, et des paliers modifiés sous un créneau en cours le replacent par sa durée et non par son indice, un indice stocké voulant dire autre chose dès que les paliers bougent. La durée configurée est alignée sur le premier palier à l'écriture : deux endroits prétendant dire ce que fait le premier appui, c'est ainsi qu'ils finissent par se contredire. Tout ce qui n'a pas de paliers garde exactement la règle 3 de la spec 174.
- Fonctionnalité (energy) : **un compteur alimenté par un autre réseau sort de la réconciliation** (spec 179, #909). La spec 173 avait déclaré une topologie que l'arithmétique de réconciliation ne voyait pas, un compteur à l'intérieur d'un autre ; voici l'autre. La répartition par usage et le donut en direct calculent tous deux « Autre » comme le total principal moins la somme des sous-compteurs, ce qui ne tient que si chaque sous-compteur mesure du courant passé par le compteur principal. Une pince sur un circuit alimenté par un **second compteur du fournisseur** — le cas qui l'a soulevée est une prise de recharge VE sur un autre abonnement — casse cela de trois façons à la fois, et l'enrôlement étant une liste noire, elle entre par défaut dans toutes les réconciliations : la répartition affiche une part que le total principal n'a jamais portée, le résidu est mangé à hauteur de la pince, à 0 sur le site de référence, et la spec 123 facture au tarif principal les kilowattheures d'un autre réseau. Même principe de conception que la spec 173, déclarer ce qui est vrai du tableau électrique plutôt que cocher une case « masquer » : un équipement dit désormais qu'il pend d'un autre réseau, et les deux surfaces l'affichent **à part** — son propre groupe, séries brutes, en kilowattheures seulement, jamais en euros — tandis que sa carte, son historique et ses graphiques ne bougent pas. Le drapeau est refusé sur le compteur principal et sur les types de production, la référence ne pouvant pas être hors d'elle-même, et il est vérifié sur le couple résultant type + drapeau, pour qu'un changement de type ultérieur ne puisse pas atteindre l'état interdit. Un compteur sur alimentation séparée ne peut pas non plus être parent au sens de la spec 173 : rien de ce que la répartition affiche ne peut être « dans » un compteur d'un autre réseau, donc une déclaration d'imbrication qui pointe dessus est conservée et simplement inutilisée, et retirer le drapeau restaure la soustraction telle quelle. L'agrégation de zone reste volontairement brute : le garage tire réellement ces watts, quel que soit le compteur qui les facture. Les installations existantes ne changent pas.
- Fonctionnalité (auth) : **un utilisateur standard choisit quel mode est actif** (issue #912, #916). La spec 131 avait classé les modes comme de la configuration en bloc, et son tableau de cas limites actait un 403 pour un utilisateur standard qui active un mode. Cela confondait deux choses que la spec sépare ailleurs : ce qu'un mode **est**, et quel mode est **actif**. Le même utilisateur avait déjà, une ligne plus haut dans la liste blanche, le droit d'une commande de zone qui éteint toutes les lumières de la maison : refuser « mettre la maison en Nuit » à côté de cela n'était pas une frontière cohérente, et cela empêchait un conjoint ou un adolescent de faire la seule chose qu'ils font le plus souvent. Activer, désactiver et appliquer à une zone sont désormais accessibles à un utilisateur standard. L'argument honnête est la délégation, pas l'innocuité : activer un mode fait plus que lever un drapeau, puisque les impacts d'un mode peuvent activer, désactiver ou reparamétrer une instance de recette, et que la désactivation ne défait pas cela. Mais c'est un administrateur qui rédige ces impacts : un utilisateur standard choisit **quand** un mode s'exécute, jamais **ce qu'il fait** — la même délégation que portent depuis toujours le calendrier et un bouton physique lié à un mode, dont aucun ne demande de rôle. Conditionner les actions de recette au rôle de celui qui active était l'alternative, et elle est pire : le même mode s'appliquerait à moitié selon qui a appuyé. Ce qu'un mode est reste réservé à l'administrateur, création, renommage, suppression et routes d'impact inchangées, et les contrôles d'ajout, d'édition et de suppression restent derrière le test admin dans l'interface. L'application à une zone gagne la ligne d'audit qui lui manquait, étant celle des trois qui envoie des ordres sans laisser de trace.
- Correctif (energy) : **la prévision solaire est notée sur l'énergie du jour, pas sur une moyenne de puissances horaires** (issue #907, #908). Le chiffre de précision affichait « ± 73 W sur 168 heures » à côté d'un pic de 3,4 kW et d'une journée de 23 kWh, ce qui a motivé le signalement. L'unité était juste, les deux côtés de la comparaison étant des puissances moyennes horaires, mais la métrique ne l'était pas : moyennée sur toutes les heures appariées, elle est dominée par la nuit, où les deux côtés valent zéro et où l'erreur est trivialement parfaite. Sur le site de référence, c'est 74 heures sur 168, ce qui divise le chiffre par deux, et la dilution est saisonnière : le score s'améliore tout seul chaque automne et ne peut pas être comparé à lui-même. Le chiffre de tête est désormais l'erreur sur l'intégrale d'énergie journalière, en kilowattheures et en part de ce qui a réellement été produit, sur des journées locales complètes : 1,15 kWh et 5,2 % sur la même installation, contre un biais cumulé de −0,7 %. Une intégrale ne peut pas être diluée par des heures qui ne produisent rien. La journée en cours n'est pas écartée pour cause d'incomplétude mais rapportée sur sa propre ligne, les deux côtés sommés sur les heures écoulées, seule façon équitable de la comparer et seule réponse à « est-ce que la journée est dans les clous » — et elle nomme sa source, la prévision émise la veille, car les deux chiffres « Aujourd'hui » viennent de prévisions différentes sous le même mot. Le pourcentage est omis plutôt qu'affiché à 0 % quand rien n'a été produit dont prendre une part, un onduleur mort sous une prévision vivante ayant l'air irréprochable au moment précis où le chiffre doit crier. Le sélecteur de fenêtre gagne 1 jour et 3 jours et s'ouvre sur 3, une semaine de points horaires étant une palissade ; la fenêtre de précision a un plancher serveur à sept jours, pour qu'un zoom ne puisse pas effacer le score ; et la courbe prévisionnelle est bornée à la fenêtre choisie, si bien qu'un zoom d'un jour n'oppose plus un jour d'historique à cinq de prévision.
- Correctif (ui) : **une icône choisie à la main suit son équipement, et la tuile temporisée cesse de rogner son propre bouton** (#910). Une icône de widget personnalisée était dessinée partout à partir des props d'aperçu figées qui existent pour la vignette du sélecteur, si bien que chaque surface l'affichait dans un état fixe : le compresseur et l'imprimante 3D ajoutés en v1.66.0 restaient au repos quoi que fasse la prise, et une prise à qui l'on donnait l'icône « Prise » s'affichait allumée pour toujours. Un dessin choisi à la main hérite désormais des props d'état de l'icône de type qu'il remplace, appliquées une seule fois dans le résolveur depuis la valeur même qui pilote la pastille d'état : l'icône et la pastille ne peuvent plus se contredire, et il n'y a pas de seconde règle marche/arrêt à tenir à jour. Les booléens sont transposés d'une famille à l'autre, le sélecteur proposant toutes les icônes sous « Autres icônes », tandis que l'état numérique est laissé tel quel, un niveau et une position ne partageant pas le même espace de valeurs. L'imprimante 3D garde sa prop à quatre valeurs, une prise ne sachant jamais que si la machine est alimentée. Par ailleurs, la tuile temporisée de la spec 174 empilait son illustration, sa pastille d'état et sa ligne de contrôles 14 px sous le bas de la coque partagée de 160 px, dont l'`overflow-hidden` coupait alors le bouton d'armement en deux — la seule chose de la tuile sur laquelle on est censé appuyer. Les lignes ont désormais une priorité : l'illustration est la seule chose sacrifiée à la largeur d'un téléphone, l'état et les contrôles atterrissent toujours dans la carte. Sur une tuile desktop de 240 px, rien ne bouge.
- Maintenance (core) : **une extension `.js` manquante est refusée par le linter, plus par le conteneur** (#924). Le backend est un module ES et s'exécute avec `node dist/index.js` : Node exige donc une extension explicite sur chaque spécificateur relatif — mais `moduleResolution: "bundler"` fait accepter l'absence par `tsc`, et vitest la résout aussi. Un `.js` manquant compile, passe le lint, passe les 2556 tests backend, puis lève `ERR_MODULE_NOT_FOUND` au démarrage du conteneur. La revue du thermostat ci-dessus en a attrapé un exactement ainsi, à la main, sur un module partagé que l'interface importe aussi ; rien dans la CI ne l'aurait vu, et rien n'aurait vu le suivant. Une règle eslint refuse désormais un import relatif, une réexportation nommée ou un `export *` sans extension, dans le backend seulement, imports de type inclus : retirer le mot-clé `type` d'un import exempté est précisément l'édition que personne ne relit.
- Maintenance (ci) : **trois hooks de machine de développement qui échouaient pour des raisons étrangères au changement poussé** (#917, #918, #920). Le contrôle de l'index des specs lançait deux processus par dossier de spec et par index, soit 700 à 175 dossiers, 13 s sur macOS et le dépassement du timeout par défaut de vitest — donc `npm run validate` et le hook de pre-push étaient rouges sur une machine de développement pendant que la CI restait verte sous Linux, et un hook qui échoue pour une raison étrangère est un hook que l'on apprend à contourner. Un test d'appartenance à un ensemble n'a pas besoin d'un processus par élément : 13 s sont devenues 0,06 s, et deux scripts frères de même forme ont suivi, `check-specs-index.sh` complet passant de 1,81 s à 0,07 s et `check-specs-complete.sh` de 1,87 s à 0,14 s. Le comportement est inchangé et les quatorze tests qui couvrent les deux assertions passent toujours. Le scan de secrets, lui, testait la présence de la CLI Docker avant de s'y rabattre, ce qui est vrai sur toute machine où Docker Desktop est installé et arrêté : le repli échouait alors à se connecter et bloquait chaque commit tant que Docker n'était pas démarré. Il interroge désormais le démon, la question la moins chère qui veuille vraiment dire « puis-je lancer un conteneur maintenant ».
- Maintenance (docs) : **les captures de la documentation sont prises sur une instance locale jetable** (#914, #915). Le pipeline de captures dépendait d'un Raspberry Pi de démonstration joignable en SSH ; une machine qu'il faut garder vivante, joignable et à jour pour un besoin de quelques fois par an est un passif, chaque session commençait par la réparer, et elle a fini par disparaître. Elle est remplacée par un Sowel jetable sur la machine du développeur, avec son propre port, son propre nom de projet et ses propres volumes, pour qu'un démontage ne puisse rien toucher d'autre, et sans socket Docker monté, ce montage donnant un accès root effectif et l'auto-mise à jour n'étant pas nécessaire ici. Le guide destiné aux agents nomme par ailleurs les trois dépôts du démonstrateur public et la carte de projet qui porte ses décisions, avec la règle qui garde le produit propre : la démo ne justifie jamais à elle seule un changement du cœur.
- Maintenance (plugins) : panasonic_cc 2.3.2 dans le registre (#906), la version compagnon de la spec 176.
