...
describe("EquipmentManager", () => {
  ...
  describe("executeOrder", () => {
    ...
    it("resolves null with gateTriggerMode 'toggle': no prior value known -> true, same as fixed default", async () => {
      // First-ever trigger — nothing to invert against yet, matches "fixed".
      const zone = zoneManager.create({ name: "Garage" });
      const eq = manager.create({ name: "Porte", type: "gate", zoneId: zone.id });
      const { orderIds } = seedDevice(db, {
        name: "MINI-ZBD-toggle-fresh",
        orderKeys: [
          {
            key: "state",
            type: "boolean",
      ...
      manager.addOrderBinding(eq.id, orderIds[0], "command");
      manager.update(eq.id, { gateTriggerMode: "toggle" });
    ...
    it("resolves null with gateTriggerMode 'toggle': last known state true -> sends OFF (issue #627)", async () => {
      // Reproduces the real-hardware finding: a SONOFF MINI-ZBD with inching
      // never reports its auto-off, so the cached "state" stays stuck at true
      // after the first trigger — resending a fixed "ON" is silently dropped.
      // Reading the last known value and sending its inverse fixes it.
      const zone = zoneManager.create({ name: "Garage" });
      const eq = manager.create({ name: "Porte", type: "gate", zoneId: zone.id });
      const { orderIds } = seedDevice(db, {
        name: "MINI-ZBD-toggle-on",
        dataKeys: [{ key: "state", type: "boolean", value: "true" }],
        orderKeys: [
          {
            key: "state",
            type: "boolean",
      ...
      expect(JSON.parse(mockPublished[0].payload)).toEqual({ state: "OFF" });
    });

    it("resolves null with gateTriggerMode 'toggle': last known state false -> sends ON", async () => {
      const zone = zoneManager.create({ name: "Garage" });
      const eq = manager.create({ name: "Porte", type: "gate", zoneId: zone.id });
      const { orderIds } = seedDevice(db, {
        name: "MINI-ZBD-toggle-off",
        dataKeys: [{ key: "state", type: "boolean", value: "false" }],
        orderKeys: [
          {
            key: "state",
            type: "boolean",
    ...
    it("gateTriggerMode 'fixed' (default) always sends ON regardless of last known state (regression guard)", async () => {
      const zone = zoneManager.create({ name: "Garage" });
      const eq = manager.create({ name: "Porte", type: "gate", zoneId: zone.id });
      expect(eq.gateTriggerMode).toBe("fixed");
      const { orderIds } = seedDevice(db, {
        name: "MINI-ZBD-fixed",
        dataKeys: [{ key: "state", type: "boolean", value: "true" }],
        orderKeys: [
      ...
      manager.addOrderBinding(eq.id, orderIds[0], "command");
      // gateTriggerMode left at its default ("fixed") — not opted in.

      await manager.executeOrder(eq.id, "command", null);
      expect(JSON.parse(mockPublished[0].payload)).toEqual({ state: "ON" });

      await manager.executeOrder(eq.id, "command", null);
      expect(JSON.parse(mockPublished[1].payload)).toEqual({ state: "ON" });
    });

    it("round-trips gateTriggerMode through update", () => {
      const zone = zoneManager.create({ name: "Garage" });
      const eq = manager.create({ name: "Porte", type: "gate", zoneId: zone.id });
      expect(eq.gateTriggerMode).toBe("fixed");

      manager.update(eq.id, { gateTriggerMode: "toggle" });
      expect(manager.getById(eq.id)?.gateTriggerMode).toBe("toggle");

      manager.update(eq.id, { gateTriggerMode: "fixed" });
      expect(manager.getById(eq.id)?.gateTriggerMode).toBe("fixed");
    });
