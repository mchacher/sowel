...
export function registerCalendarRoutes(app: FastifyInstance, deps: CalendarDeps): void {
  ...
  app.post<{
    Params: { id: string };
    Body: { days: number[]; time: string; modeIds: string[] };
  }>("/api/v1/calendar/profiles/:id/slots", async (request, reply) => {
    const { days, time, modeIds } = request.body ?? {};
    if (!Array.isArray(days) || !time || !Array.isArray(modeIds)) {
      return reply.code(400).send({ error: "days, time, and modeIds are required" });
    }

    try {
      const slot = calendarManager.addSlot(request.params.id, days, time, modeIds);
